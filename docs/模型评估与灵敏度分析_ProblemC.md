...
**合理性**：投票份额均值为 0.1207、标准差 0.0759、最小值 0、最大值 0.6667，区间与参赛人数变化相符。跨周平滑偏好后，赛季内周际平均绝对变化均值为 0.0430，中位数 0.0375（见 [outputs/eval_q1_by_season.csv](../outputs/eval_q1_by_season.csv)），体现序列稳定性。  
...
**稳定性与鲁棒性**：周际变化在赛季之间存在差异（最大值 0.1176），但整体处于低—中幅度范围，说明估计序列稳定且不会出现无依据的剧烈跳动。  
**适用范围与局限**：无人淘汰或多人淘汰周次仍需跳过；最小值为 0 反映部分周次存在极低份额估计，表明在约束强或样本规模小的周次估计可能更集中。

---

## 问题二：规则比较与争议反事实
**有效性**：周次层面共比较 333 周，差异周次 1 个，差异比例 0.00300（见 [outputs/eval_q2_summary.csv](../outputs/eval_q2_summary.csv)），说明模型能实现规则对照目标并量化分歧。  
**合理性**：差异比例非均匀分布，符合“规则作用随赛季结构变化而异”的背景判断。  
**灵敏度**：在低/高裁判评分离散度分组中，差异比例分别为 0.00599 与 0（见 [outputs/eval_q2_sensitivity.csv](../outputs/eval_q2_sensitivity.csv)），差异幅度有限，说明规则分歧对评分波动的敏感性偏低。  
...
## 问题三：舞者与选手特征影响
**有效性**：基准拟合度为 $R^2_{judge}=0.3467,\ R^2_{fan}=0.1617$，说明结构变量对裁判评分解释度高于投票份额。  
**合理性**：舞者固定效应与选手特征同时进入，可分离技术与人气影响，符合背景中“裁判评分更技术、观众投票更主观”的逻辑。  
**灵敏度**：留一季重估显示 $R^2_{judge}$ 均值 0.3492、$R^2_{fan}$ 均值 0.1638，标准差分别为 0.0040 与 0.0031（见 [outputs/eval_q3_leave_one_season.csv](../outputs/eval_q3_leave_one_season.csv)），表明模型对样本扰动稳定。  
**稳定性与鲁棒性**：舞者系数的跨季标准差均值为 0.1588（裁判）与 0.00732（投票）（见 [outputs/eval_q3_coef_stability.csv](../outputs/eval_q3_coef_stability.csv)），显示裁判得分效应更敏感、投票效应更平稳，符合两类结果的性质差异。  
